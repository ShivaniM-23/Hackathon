...
function riskTone(riskLevel?: string) {
  if (riskLevel === "LOW") return "border-emerald-500/20 bg-emerald-500/10 text-emerald-200";
  if (riskLevel === "HIGH") return "border-red-500/20 bg-red-500/10 text-red-200";
...
export default function ChatPanel({ jobId, companyName, trustScore, riskLevel }: Props) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <header className="border-b border-slate-800 bg-slate-950/70 p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-white">AI Investigator</h2>
        ...
        {messages.map((message, index) => (
          <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[88%] ${message.role === "user" ? "text-right" : "text-left"}`}>
              {message.role === "ai" && (
                <div className="mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase text-slate-500">
      ...
      <div className="border-t border-slate-800 p-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            disabled={loading}
            className="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-cyan-400"
          />
          <button
            onClick={() => send(input)}
            disabled={loading || !input.trim()}
            className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-cyan-400 text-slate-950 transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
